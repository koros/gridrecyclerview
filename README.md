...
Handle grid cell clicks directly in the composable you render for each item:
    ...
    gridItemContent = { header, item ->
        ...
        MovieCard(
            ...
            modifier = Modifier.clickable {
                // header identifies the section, movie is the clicked cell data
            }
        )
    }
)
...
Header and subheader clicks follow the same pattern: add `Modifier.clickable` inside `headerContent` to the exact title, subtitle, icon, or action you want to handle. In the legacy RecyclerView API, set grid cell, header, and subheader click listeners inside your own `ViewHolder` implementations.
